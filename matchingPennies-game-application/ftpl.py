...
NUM_STEPS = 10000
lr = 0.01
K = 1  # size of the queue of strategy
...
x_rate = 3 # rate of the exponential distribution perturbation
y_rate = 2
...
x_optimizer = optim.SGD(x_player.parameters(), lr=lr)
y_optimizer = optim.SGD(y_player.parameters(), lr=lr)
...
for t in range(NUM_STEPS):
    ...
    for param in y_queue:
        ...
        y = y_player()
        y_loss -= torch.mm(torch.mm(x, A), y.T)
    y_loss /= len(y_queue)
...
plt.show()
plt.savefig("ftpl")
print("saved under the ftpl.png")
