...
for t in range(NUM_STEPS):
    # mediator proposes a code based on the noisy observations.
    ...
    for param in y_queue:
        ...
        y = y_player(code)
        loss = torch.relu(-torch.mm(torch.mm(x, A), y.T))
...
plt.ioff()
plt.plot()
plt.savefig('ftnpl_nonconvex')
print("figure saved under the ftnpl_nonconvex.png")
