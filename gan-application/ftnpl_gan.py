...
class Mediator(nn.Module):
    ...
    def __init__(self,ngpu):
        ...
        self.embed_mean = nn.Linear(81, nmc)
        self.embed_log_std = nn.Linear(81, nmc)

        self.action_scale = torch.tensor(1)
        self.action_bias = torch.tensor(0.0)

    def forward(self, input):
        x = self.main(input).view(batch_size,-1)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        ###########################
        codes, log_prob, mean = netM.act(real_cpu)
        #codes = mean.detach()
        codes = codes.detach()
        ...
        for param in D_queue:  # Note that D_x and D_G_z1 at the end are corresponding with the latest strategy.
            ...
            noise = torch.cat([noise, codes.reshape(batch_size, nmc, 1, -1)], dim=1)
